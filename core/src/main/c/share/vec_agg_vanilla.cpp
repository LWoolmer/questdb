...
/*
https://developer.arm.com/documentation/100891/0611/coding-considerations/using-sve-intrinsics-directly-in-your-c-code

Some useful things to know about SVE intrinsics:

Types:
  - sv_<type>: a vector of <type>
  - 

Predication Suffixes:
  - _z : Set all inactive elements to 0 in the result
  - _m : Copy all inactive elements from the first argument
  - _x : Don't care
...
int64_t countInt_Vanilla(int32_t *pi, int64_t count) {
    ...
    int64_t cnt = 0;
    for (int64_t i = 0; i < count; i += svcntw()) {
        svbool_t  prd = svwhilelt_b32(i, count);    
        svint32_t vec = svld1(predicate, &pi[i]);
        svbool_t  cmp = svcmpne_n_s32(predicate, vec, I_MIN);
        cnt += svcntp_b32(predicate, cmp)
    }
    return cnt;
}
...
int64_t countLong_Vanilla(int64_t *pl, int64_t count) {
    ...
    int64_t cnt = 0;
    for (int64_t i = 0; i < count; i += svcntw()) {
        svbool_t  prd = svwhilelt_b64(i, count);    
        svint64_t vec = svld1(predicate, &pi[i]);
        svbool_t  cmp = svcmpne_n_s64(predicate, vec, L_MIN);
        cnt          += svcntp_b64(predicate, cmp)
    }
    return cnt;
}
...
int64_t countDouble_Vanilla(double *d, int64_t count) {
    ...
    for (int64_t i = 0; i < count; i += svcntw()) {
        svbool_t    prd = svwhilelt_b64(i, count);    
        svfloat64_t vec = svld1(predicate, &pi[i]);
        svbool_t    cmp = svcmpne_f64(predicate, vec, vec);
        cnt            += svcntp_b64(predicate, cmp)
    }
    return cnt;
}
